...
:END:
The steps for importing an SSH key into GPG Depends on the type of ssh
key. Below are instructions for importing RSA and ED25519 ssh keys
into gpg.
...
:CUSTOM_ID: ed25519
:END:
[[https://superuser.com/questions/1414381/how-to-import-an-ssh-ed25519-key-to-gpg][import ed25519 ssh key]]

#+begin_src sh
  ssh-add <private-key-file>
#+end_src

Identify the keygrip of the just added key.
#+begin_src sh
  gpg -K --with-keygrip
...
Add the new key as a subkey to an already existing key
1. ~gpg --expert --edit-key <existing-gpg-id>~
2. ~addkey~
3. Select "Existing Key" (Maybe "13")
4. Enter the keygrip identified earlier
5. Set the capabilities to "Authenticate"
6. Answer questions appropriately
...
:CUSTOM_ID: rsa
:END:
follow [[https://opensource.com/article/19/4/gpg-subkeys-ssh-multiples][import RSA ssh keys]]

* TODO Create SSH Keys with GPG
:PROPERTIES:
...
:END:
If you have multiple authentication keys you want to use for ssh then
you need to add their public keys to =~/.ssh= so you can reference
them in =~/.ssh/config=. You also need to add the gpg subkey
fingerprints to =~/.gnupg/sshcontrol= so the gpg-agent knows which
keys can be used for ssh.
...
- [[https://wiki.archlinux.org/title/GnuPG#SSH_agent][Arch Wiki]]
